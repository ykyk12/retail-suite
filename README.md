...
| 版本 | 说明 |
|---|---|
| 1.7.0 | **第二轮深度升级（见 11.2）**：修两个真 bug——①收银幂等快路径越权（`selectByRequestId` 不带 `store_id`，跨门店撞 requestId 会把别家订单详情泄露给本店，改为按本店过滤、跨店冲突直接 409）；②退货超额（整单折扣下退货按行挂牌价全退且无封顶，顾客只付 150 却可能退 200，改为按实收比例分摊折扣并封顶于实收金额）。新增**低库存预警工单闭环**（出库砸破阈值自动开 OPEN 工单，同商品不重复开，补货/盘点后手动闭环）。后端测试 84 → 92（+8） |
...
### 11.2 第二轮深度升级（v1.7.0）详情

**走查方法：** 通读收银/退货/库存/采购/报表/管家全链路，逐项核对浮点精度、越权 IDOR、SQL 拼接、并发竞态、事务边界、异常吞掉、资源未关闭、DTO 校验、幂等重复提交。每个认定的 bug 都先写一个能复现的红灯回归测试，再修复转绿。

**修掉的两个真 bug：**

1. **收银幂等快路径的跨门店越权（IDOR）**
   - 位置：`SaleOrderMapper.selectByRequestId` 与 `SaleService.checkout`。
   - 触发：`uk_sale_request_id` 是**全局**唯一索引，而幂等快路径只按 `request_id` 查、不带 `store_id`。门店 A 落了一笔 `requestId=R` 后，门店 B 用同一个 `R` 来收银，快路径直接把门店 A 的订单（客户名、商品、金额）返回给门店 B——跨租户数据泄露。
   - 修法：幂等查询带上 `store_id`；并发重复提交回查也限定本店；若唯一索引冲突回查不到本店订单，说明撞的是**别的门店**的 requestId，直接抛 `CONFLICT`，既不泄露也不静默放过。
   - 回归测试：`SaleMoneyAndTenantTest.跨门店使用相同requestId_不得返回别家订单`。

2. **退货超额（钱箱倒亏）**
   - 位置：`SaleService.refund`。
   - 触发：订单有整单折扣时（应收 200、折扣 50、实收 150），旧逻辑按行挂牌价全额累加退款，整单退完 `refund_amount=200 > 实收 150`，顾客拿回的比他付的还多，且没有任何"退款不超过实收"的兜底。
   - 修法：折扣是整单级的，退货按"实收/应收"比例分摊到每一行；再用 `payAmount - 已退` 兜底封顶，多次部分退款的舍入误差被吸收，整单全退恰好等于实收。零折扣订单行为完全不变（回归零风险）。
   - 回归测试：`整单折扣时整单退款不超过实收金额`、`有折扣时部分退款按比例分摊折扣`、`整单全免时退款金额为零不为负`。

**新增功能：低库存预警工单闭环**
- 此前"低库存"只是只读列表；现在出库（收银/报损）把库存砸破阈值时，**在同一事务内自动开一条 OPEN 工单**（新表 `stock_alert`，含触发时库存快照与阈值），同一商品挂着 OPEN 工单不重复开，店长补货/盘点后 `POST /api/inventory/alerts/{id}/resolve` 闭环——把"看到缺货"变成"有人负责、处理完闭环"。
- 新指标 `app.stock_alert.opened` 接入 `/actuator/metrics`；MySQL 增量迁移见 `deploy/mysql/migration/v1.7.0-stock-alert.sql`。
- 测试：`StockAlertTest`（开单/去重/闭环后再开/重复闭环冲突）。

**验证结果：**
- `mvn -B test` → **Tests run: 92, Failures: 0, Errors: 0（BUILD SUCCESS）**，原 84 个用例全部保持绿，新增 8 个。
- 依赖：未跨大版本升级，保持离线 `.m2` 可复现；未引入新的重型依赖。
