...
当前版本 **1.2.0**：后端 70 个自动化测试、前端类型检查与构建、CI 三个 job 全绿（含 docker compose 全栈冒烟 40 项）；`docker compose up -d --build` 一键起全栈（MySQL + Redis + 后端 + Nginx 前端）。
...
| 版本 | 说明 |
|---|---|
| 1.2.0 | 管家主动巡检：每天开门前自动巡检（过期/临期/断货/补货/滞销/毛利异常/账实不符/批次不符）→ 结构化日报落库 → 补货建议一键转采购草稿；新增毛利异常工具（第 10 个工具）；补货与滞销口径抽成单一口径服务（对话与日报结论必然一致）；e2e 冒烟扩到 41 项 |
