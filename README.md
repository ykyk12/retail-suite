...
当前版本 **1.4.1**：后端 73 个自动化测试（含 22 条规则兜底评测用例）、前端类型检查与构建、CI 三个 job 全绿（含 docker compose 全栈冒烟 42 项）；`docker compose up -d --build` 一键起全栈（MySQL + Redis + 后端 + Nginx 前端）。已在 Windows 11 + Docker Desktop（WSL2 后端）实机跑通 42/42。
